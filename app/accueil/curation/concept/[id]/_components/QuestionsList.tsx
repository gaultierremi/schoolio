...
import QuestionHintsPanel from "./QuestionHintsPanel";
import type { MisconceptionRow, QuestionRow } from "../types";
...
export default function QuestionsList({
  ...
  // S5-2 : Set des question IDs avec le panel hints ouvert (lazy fetch)
  const [expandedHintsIds, setExpandedHintsIds] = useState<Set<string>>(new Set());
  ...
  return (
    ...
    >
      {questions.map((q) => {
        ...
        const isHintsOpen = expandedHintsIds.has(q.id);
        return (
          <li key={q.id} aria-busy={isPending || undefined} className="p-4">
            <div className="flex items-start gap-4">
              <div className="min-w-0 flex-1">
                ...
                <button
                  ...
                  aria-controls={`hints-panel-${q.id}`}
                  className="
                ...
                >
                  ...
                  {isHintsOpen ? "Masquer les indices" : "Voir/éditer les indices"}
                </button>
              </div>
            ...
            {isHintsOpen ? (
              <div id={`hints-panel-${q.id}`} className="mt-3 pl-4 border-l-2 border-indigo-200 dark:border-indigo-900">
                <QuestionHintsPanel
                  questionId={q.id}
