...
22. *Update ton statut agent automatiquement à chaque changement d'état.* Mission Control V2 expose `POST /api/admin/agent-status` pour `Claudy`, `Coco`, `Claudia`, `Jules`. Statuts autorisés : `working`, `planning`, `blocked`, `idle`, `done`.

Update OBLIGATOIRE :
- Au début d'une tâche → `status="working"` ou `status="planning"`.
- Si tu attends une validation utilisateur → `status="planning"`.
- Si tu es bloqué (conflict, erreur, attente input) → `status="blocked"`.
- À la fin d'une tâche (commit pushé + PR ou merge fait) → `status="idle"`.

Utilise le helper non-bloquant :
.\scripts\update-status.ps1 -name Claudy -status working -task "Session 3" -branch "feat/..." -eta "~2h"
